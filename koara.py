...
class Koara:
    ...
    def do_fenced_code_blocks(self, text):
        return re.sub(r'(?m)^`{3}(.*)\n([\s\S]+?)`{3}', lambda m:
                      self.renderer.render_fenced_code_blocks(self.encode_code(m.group(2).strip()), m.group(1)), text)
    ...
    def do_unordered_lists(self, text, nested):
        return re.sub(r'(?m)^[ \t]{0,3}((\*[ \t]+[\s\S]+?)(?=\n{3,}|\n{2}[^\*\s]|\n+\d[.])|\*[ \t]+[\s\S]+$)', lambda m:
                      self.renderer.render_list(self.do_list_items(m.group(1)), nested, False), text)

    def do_ordered_lists(self, text, nested):
        return re.sub(r'(?m)^[ \t]{0,3}((\d+[.][\s\S]+?)(?=\n{3}|\n{2}<ul>|\n{2}[^(\d\s)]|\n{2}\d[^.])|\d+[.][\s\S]+$)', lambda m:
                      self.renderer.render_list(self.do_list_items(m.group(1)), nested, True), text)
    ...
    def do_images(self, text):
        ...
        text = re.sub(r'!\[(.+)\][ ]?\((\S+)[ ]*\"(.*)\"[ ]*\)', lambda m:
                      self.renderer.render_image(m.group(2), m.group(3), m.group(1)), text)
        text = re.sub(r'!\[(.+)\][ ]?\((.+)\)', lambda m:
                      self.renderer.render_image(m.group(2), None, self.hash_inline(m.group(1))), text)
    ...
    def do_links(self, text):
        ...
        return re.sub(r'(\[(.*?)\][ ]?\([ \t]*<?(.+?)>?[ \t]*(([\'\"])(.*?)\5)?\))', lambda m:
                      self.renderer.render_link(m.group(3), m.group(6), m.group(2)), text)

    def do_auto_links(self, text):
        if('autolinks' in self.extensions):
            text = re.sub(r'(?m)(\()?((https?|ftp):\/\/[\w./&;]*)\b(?=\s|$)', lambda m:
                          self.renderer.render_link(m.group(2), None, m.group(2)), text)
            text = re.sub(r'(?m)\b([-.\w]+\@[-a-z0-9]+(\.[-a-z0-9]+)*\.[a-z]+)\b(?=\s|$)', lambda m:
                          self.renderer.render_link("mailto:" + m.group(1), None, m.group(1)), text)
        return text
    ...
    def hash_html(self, literal):
        hash = "===KOARA_HTML~" + str(len(self.htmlHash)) + "==="
        self.htmlHash[hash] = literal
        return hash

    def hash_inline(self, literal):
        hash = "===KOARA_INLINE~" + str(len(self.inlineHash)) + "==="
